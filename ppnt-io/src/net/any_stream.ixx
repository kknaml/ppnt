...
export namespace ppnt::net {
    ...
    struct AnyStream {
        ...
        virtual auto read(std::span<uint8_t> buf) -> io::Task<Result<size_t>> = 0;
        virtual auto write(std::span<const uint8_t> buf) -> io::Task<Result<size_t>> = 0;
    ...
    struct AnyStreamImpl : AnyStream {
        ...
        auto read(std::span<uint8_t> buf) -> io::Task<Result<size_t>> override { co_return co_await inner.read(buf); }
        auto write(std::span<const uint8_t> buf) -> io::Task<Result<size_t>> override { co_return co_await inner.write(buf); }
